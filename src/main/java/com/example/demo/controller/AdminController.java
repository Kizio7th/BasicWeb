...
public class AdminController {
    ...
    public String addBook(@ModelAttribute("book") BookDto bookDto, Model model,
            @RequestParam("photo") MultipartFile bookImage, Authentication auth, BindingResult result) {
    ...
    public String updateBook(Model model, @ModelAttribute("book") BookDto bookDto, @PathVariable("id") Long id,
            Authentication auth, @RequestParam("photo") MultipartFile bookImage, BindingResult result) {
        List<BookDto> existingBook = bookService.findBooksByTitleAndAuthor(bookDto.getTitle(), bookDto.getAuthor());
        if (existingBook != null && !existingBook.isEmpty()) {
        ...
        if (result.hasErrors()) {
            if (bookImage.isEmpty()) {
                ...
                bookDto.setCover(bookService.handleMultipartFile(bookImage));
    ...
    public String paid(Model model) {
        ...
        model.addAttribute("paids", orderService.BillToPair(bills));
        return "paid";
    }
}
